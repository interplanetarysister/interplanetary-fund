...
export default function App() {
  ...
  const [showTempGate, setShowTempGate] = useState(false);
  const [pinInput, setPinInput] = useState("");
  const [pinError, setPinError] = useState(false);
  const [authed, setAuthed] = useState(false);
  const [tempAuthed, setTempAuthed] = useState(false);
  ...
  const handleLogoTap = useCallback(() => {
    ...
    if (newCount >= 5) {
      if (authed || tempAuthed) setView("admin");
      else if (tempAuthed) setShowTempGate(true);
      else setShowPinGate(true);
      setTapCount(0);
    } else {
      setTapCount(newCount);
    }
  }, [tapCount, authed, tempAuthed]);
  ...
  return (
    <TermsAcceptance>
      <div className="min-h-screen bg-ifdark">
        ...
        {showPinGate && (
          <div className="fixed inset-0 z-50 bg-ifdark/95 backdrop-blur flex items-center justify-center">
            <div className="max-w-xs w-full px-6">
              ...
              <input type="password" inputMode="numeric" maxLength={8} placeholder="••••" value={pinInput} onChange={(e) => { setPinInput(e.target.value.replace(/\D/g, "")); setPinError(false); }} className="input-field text-center text-xl tracking-[0.3em] font-bold" autoFocus />
        ...
        {showTempGate && (
          <div className="fixed inset-0 z-50 bg-ifdark/95 backdrop-blur flex items-center justify-center px-4">
            <div className="max-w-xs w-full">
              <TempAdminAccessGate onUnlock={() => {
                setTempAuthed(true);
                setShowTempGate(false);
                setView("admin");
              }} />
              <button onClick={() => setShowTempGate(false)} className="w-full text-xs text-ifmuted text-center mt-3 py-2">Cancel</button>
            </div>
          </div>
        )}
